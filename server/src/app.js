...
app.post('/api/journal', requireAuth, async (req, res, next) => { try { const entry = await Journal.create({ userId: req.user.id, title: req.body.title, content: req.body.content }); res.status(201).json({ entry }); } catch (err) { next(err); } });
app.put('/api/journal/:id', requireAuth, async (req, res, next) => { try { const entry = await Journal.findOneAndUpdate({ _id: req.params.id, userId: req.user.id }, { title: req.body.title, content: req.body.content }, { new: true, runValidators: true }); if (!entry) return res.status(404).json({ message: 'Journal entry not found.' }); res.json({ entry }); } catch (err) { next(err); } });
